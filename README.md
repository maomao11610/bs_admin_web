...
## 5 引入antd
    下载antd的包 yarn add antd / npm install antd --save
    按需打包: 只打包import引入组件的js/css   
        1. 下载工具包  yarn add react-app-rewired customize-cra babel-plugin-import
...
```package.json
  "scripts": {
    "start": "react-scripts start",  // 根据dependencies/react-scripts来运行
...
更换为  
  "scripts": {
    "start": "react-app-rewired start",  // 根据dependencies/react-app-rewired来运行
    "build": "react-app-rewired build",  // react-app-rewired这个包会使config-overrides.js里面的配置生效, 而react-scripts没这个效果
...
```
    ...
    自定义主题
        下载工具包  yarn add less less-loader  /  npm i --save-dev less@2.7.3 less-loader@5.0.0
        config-overrides.js
```js
...
module.exports = override(
    ...
    addLessLoader({
        ...
        modifyVars: {'@primary-color': '#1DA57A'},  // 配置全局主题色
    }),
);
```
    使用antd的组件
...
## 3. 实现登陆(包含自动登陆)
    login.jsx
        ...
        2). 如果失败, 显示错误提示信息
        3). 如果成功了:
            保存user到localStorge/内存中
            跳转到admin
        4). 如果内存中的user有值, 自动跳转到admin
    src/index.js
        读取localStorge中user到内存中保存
    admin.jsx
...
## 2. jsonp解决ajax跨域的原理  npm install jsonp
...
## 5. 更新分类   yarn add prop-types  --> prop-types对传入当前组件的props进行约束
    1). 界面
...
## 4. ProductHome组件
    ...
    4). 进入详情界面
       history.push('/product/detail', {product})   // 这种带{product}的方式只支持BrowserRouter
